...
router.get('/stats', authenticateToken, requireAdmin, async (req, res) => {
  try {
    // Monthly revenue (last 12 months) — PostgreSQL syntax
    ...
    const { rows: topServices } = await req.db.query(`
      SELECT s.name, COUNT(b.id) AS booking_count,
             COALESCE(SUM(s.price), 0) AS total_revenue
      FROM bookings b
      LEFT JOIN services s ON b.service_id = s.id
      GROUP BY s.id, s.name ORDER BY booking_count DESC LIMIT 10
    `);
    ...
    // Revenue this month vs last month — PostgreSQL date_trunc
    const thisMonth = new Date().toISOString().slice(0, 7);
    const lastDate = new Date(); lastDate.setMonth(lastDate.getMonth() - 1);
    const lastMonth = lastDate.toISOString().slice(0, 7);
    const thisMonthRev = (await req.db.query(
      "SELECT COALESCE(SUM(total), 0) AS val FROM invoices WHERE status != 'draft' AND TO_CHAR(created_at, 'YYYY-MM') = $1",
      [thisMonth]
    )).rows[0].val;
    const lastMonthRev = (await req.db.query(
      "SELECT COALESCE(SUM(total), 0) AS val FROM invoices WHERE status != 'draft' AND TO_CHAR(created_at, 'YYYY-MM') = $1",
      [lastMonth]
    )).rows[0].val;
