  ...
  return (
    <div style={{ maxWidth:800, margin:'0 auto', padding:'2rem 1rem' }}>
      <div style={{ textAlign:'center', marginBottom:'2rem' }}>
        <h1 style={{ fontSize:'1.8rem', fontWeight:800, color:'var(--blue-800)' }}>⭐ Customer Reviews</h1>
        <p style={{ color:'var(--gray-500)' }}>See what our customers say about Snow Bro's services.</p>
      </div>
      ...
      {stats && stats.total > 0 && (
        <div className="card" style={{ textAlign:'center', marginBottom:'2rem', background:'linear-gradient(135deg, var(--blue-50), #fff)' }}>
          ...
          <div style={{ fontSize:'.85rem', color:'var(--gray-500)', marginTop:'.25rem' }}>Based on {stats.total} review{stats.total > 1 ? 's' : ''}</div>
        </div>
      )}
